 ...
 * DE: Hilfsklasse zur Verschluesselung eines Klar-Text Passwortes als SHA1
 * Hash.<br>
 * Zur Verschluesselung wird zusaetzlich ein vorgegebenes 'salt' benutzt um das
 * Passwort sicherer zu gestalten.<br>
 * ----------------------------------------------------------------------------<br>
 * 
 * @see /src/main/resources/securitySalt.properties
 * @author Stephan Gerth
 */
public class OXSHA1Utils implements Serializable {
	 ...
	 * 
	 * @return a pair of password and the encrypted hash.
	 *         result.put("rawPassword", rawPassword);<br>
	...
	public static String encrypt(CharSequence rawPass) {
        ...
        try {
            ...
            d.reset();
            d.update(rawPass.toString().getBytes("UTF-16"));
        } catch (Exception e) {
                e.printStackTrace();
        }        
	...
	public static String hashPasswordDefault(CharSequence rawPass) {
		...
		ShaPasswordEncoder pe = new ShaPasswordEncoder(SHA_STRENGTH);
		String encPassword = pe.encodePassword(rawPass.toString(), salt);
	...
	private static String getSalt() {
		...
		try {
			...
			input = OXSHA1Utils.class.getClassLoader().getResourceAsStream(filename);
